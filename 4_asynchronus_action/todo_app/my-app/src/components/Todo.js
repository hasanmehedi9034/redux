...
export default function Todo({todo}) {
    ...
    const handleColorChange = (todoId, priorityColor) => {
        dispatch(colorSelected(todoId, priorityColor))
    }
    ...
    return (
        ...
        <div className={`select-none flex-1 ${completed &&'line-through'}`}>
            ...
            onClick={() => handleColorChange(id, 'yellow')}
                className={`flex-shrink-0 h-4 w-4 rounded-full border-2 ml-auto hover:bg-yellow-500 cursor-pointer border-yellow-500 ${color==='yellow' && 'bg-yellow-500'}`}
            ></div>
